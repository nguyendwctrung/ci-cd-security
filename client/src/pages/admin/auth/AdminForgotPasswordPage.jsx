...
function EnterEmailForm({ onNext, setSharedData }) {
  ...
  useEffect(() => {
    ...
    validator.onSuccess((event) => {
      ...
      fetch(`${API_URL}/api/admin/auth/forgot-password`, {
        method: "POST",
...
function VerifyOTPForm({ sharedData, onNext, setSharedData }) {
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    fetch(`${API_URL}/api/admin/auth/verify-otp-forgot-password`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function ResetPasswordForm({ sharedData }) {
  ...
  useEffect(() => {
    ...
    validator.onSuccess((event) => {
      ...
      fetch(`${API_URL}/api/admin/auth/reset-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-8 shadow-xl border border-white/20">
          ...
          <div className="flex items-center justify-center gap-3 mb-8">
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${step >= 1 ? "bg-purple-500 text-white" : "bg-white/20 text-gray-400"}`}
            >
              1
            </div>
            <div
              className={`w-12 h-1 rounded ${step >= 2 ? "bg-purple-500" : "bg-white/20"}`}
            ></div>
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${step >= 2 ? "bg-purple-500 text-white" : "bg-white/20 text-gray-400"}`}
            >
              2
            </div>
            <div
              className={`w-12 h-1 rounded ${step >= 3 ? "bg-purple-500" : "bg-white/20"}`}
            ></div>
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${step >= 3 ? "bg-purple-500 text-white" : "bg-white/20 text-gray-400"}`}
            >
              3
            </div>
          </div>
          ...
          {step === 3 && <ResetPasswordForm sharedData={sharedData} />}
        </div>
